...
wp core install --path="/var/www/html" --url=http://${WORDPRESS_HOSTNAME}:${WORDPRESS_PORT} --title="Local Wordpress By Docker" --admin_user=${WORDPRESS_ADMIN_USER} --admin_password=${WORDPRESS_ADMIN_PASSWORD} --admin_email=${WORDPRESS_ADMIN_EMAIL};
